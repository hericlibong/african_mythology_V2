...
def test_style_matrix_entries_have_expected_blocks():
    matrix = load_style_matrix()

    for key, entry in matrix.items():
        assert isinstance(entry, dict), f"{key}: entry must be an object"
        assert set(entry).issubset(EXPECTED_TOP_LEVEL_BLOCKS | {"inherits"}), (
            f"{key}: unexpected keys {set(entry) - (EXPECTED_TOP_LEVEL_BLOCKS | {'inherits'})}"
        )

        for block in EXPECTED_TOP_LEVEL_BLOCKS:
            assert block in entry, f"{key}: missing block {block}"

        assert isinstance(entry["visual_signature"], str) and entry["visual_signature"].strip(), (
            f"{key}: visual_signature must be a non-empty string"
        )

        for block, nested_keys in EXPECTED_NESTED_BLOCKS.items():
            nested = entry[block]
